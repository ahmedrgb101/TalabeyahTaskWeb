...
namespace TalabeyahTaskWeb.Client.Pages.Identity.Roles;
public partial class RolePermissions
{
    ...
    private Dictionary<string, List<PermissionViewModel>> _filteredgroupedRoleClaims = default!;
    ...
    private string _searchPermissionString;
    private string SearchPermissionString
    {
        get => _searchPermissionString;
        set
        {
            _searchPermissionString = value;
            SearchTaps(value);
            StateHasChanged();
        }
    }
    ...
    protected override async Task OnInitializedAsync()
    {
            ...
            is RoleDto role && role.Permissions is not null)
        {
            ...
            _filteredgroupedRoleClaims = _groupedRoleClaims;
        }
    ...
    private async void SearchTaps(string? search)
    {
        var state = await AuthState;
        if (await ApiHelper.ExecuteCallGuardedAsync(
                () => RolesClient.GetByIdWithPermissionsAsync(Id), Snackbar)
            is RoleDto role && role.Permissions is not null)
        {
            ...
            _groupedRoleClaims = permissions.Where(x=>x.Resource.ToLower().Contains(search.ToLower()))
                .GroupBy(p => p.Resource)
                .ToDictionary(g => g.Key, g => g.Select(p =>
                {
                    var permission = p.Adapt<PermissionViewModel>();
